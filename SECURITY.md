...
| The approval is an **ALLOW**, not just "few breaking changes" | Pass requires `execution_action ∈ {CONTINUE, CONTINUE_WITH_MONITORING}`. **CWM passes**; its monitoring sink is a **host claim** the gate does not verify — gate on `CONTINUE` only if you require verified monitoring. The guard side now records measured delivery evidence (`monitoring_delivery` tri-state, guard ≥8.4.0); the gate's passing set is unchanged. |
